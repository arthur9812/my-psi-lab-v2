...
# def usd_file_cfg_from_dict(dict_data : dict)->sim_utils.UsdFileCfg:
...
#     # File Cfg
#     if "scale" in keys:
#         spawn.scale = tuple(dict_data["scale"]) # type: ignore


#     if "articulation_props" in keys and dict_data["articulation_props"] is not None:
#         spawn.articulation_props = ArticulationRootPropertiesCfg()
#         spawn.articulation_props.from_dict(dict_data["articulation_props"]) # type: ignore

#     if "fixed_tendons_props" in keys and dict_data["articulation_props"] is not None:
#         spawn.fixed_tendons_props = FixedTendonPropertiesCfg()
#         spawn.fixed_tendons_props.from_dict(dict_data["fixed_tendons_props"]) # type: ignore

#     if "joint_drive_props" in keys and dict_data["articulation_props"] is not None:
#         spawn.joint_drive_props = JointDrivePropertiesCfg()
#         spawn.joint_drive_props.from_dict(dict_data["joint_drive_props"]) # type: ignore

#     if "visual_material_path" in keys and dict_data["articulation_props"] is not None:
#         spawn.visual_material_path = dict_data["visual_material_path"]

#     if "visual_material" in keys:
#         if dict_data["visual_material"]==None:
#             spawn.visual_material = None
#         else:
#             spawn.visual_material = materials.VisualMaterialCfg()
#             spawn.visual_material.from_dict(dict_data["visual_material"]) # type: ignore

#     # RigidObjectSpawnerCfg 
#     if "mass_props" in keys and dict_data["articulation_props"] is not None:
#         spawn.mass_props = MassPropertiesCfg()
#         spawn.mass_props.from_dict(dict_data["mass_props"]) # type: ignore

#     if "rigid_props" in keys and dict_data["rigid_props"] is not None:
#         spawn.rigid_props = RigidBodyPropertiesCfg()
#         spawn.rigid_props.from_dict(dict_data["rigid_props"]) # type: ignore

#     if "collision_props" in keys and dict_data["collision_props"] is not None:
#         spawn.collision_props = CollisionPropertiesCfg()
#         spawn.collision_props.from_dict(dict_data["collision_props"]) # type: ignore

#     if "activate_contact_sensors" in keys and dict_data["activate_contact_sensors"] is not None:
#         spawn.activate_contact_sensors = dict_data["activate_contact_sensors"]

#     # DeformableObjectSpawnerCfg 
#     if "deformable_props" in keys and dict_data["deformable_props"] is not None:
#         spawn.deformable_props = DeformableBodyPropertiesCfg()
#         spawn.deformable_props.from_dict(dict_data["deformable_props"]) # type: ignore

#     # Spawn Config
#     if "visible" in keys and dict_data["visible"] is not None:
#         spawn.visible = dict_data["visible"]

#     # Todo ： make sure this is right
#     if "semantic_tags" in keys and dict_data["semantic_tags"] is not None:
#         spawn.visible = dict_data["semantic_tags"]
#     return spawn

# def spawner_cfg_from_dict(dict_data : dict)->sim_utils.SpawnerCfg:
#     keys = dict_data.keys()
...
#     # Todo ： make sure this is right
#     if "semantic_tags" in keys:
#         spawn.visible = dict_data["semantic_tags"]
#     return spawn

# def merge_scene_config(scene_cfg, scene_cfg_json:dict):
#     # RigidObject，StaticObject和ReformableObject为list
#     # 仅当dict中list为空时，采用cfg默认值

#     # global light config
#     scene_cfg.global_light_cfg = AssetBaseCfg().from_dict(scene_cfg_json["global_light_cfg"]) # type: ignore
...
#     # #  robot 
#     # if "robots_cfg" in scene_cfg_json.keys():
#     #     scene_cfg.robots_cfg = dict()
#     #     for name, cfg in scene_cfg_json["robots_cfg"].items():
#     #        scene_cfg.robots_cfg[name] = RobotBaseCfg(
#     #            spawn=usd_file_cfg_from_dict(cfg["spawn"])
#     #        )
    

#     # # StaticObject
#     # if "static_object_cfg" in scene_cfg_json.keys():
#     #     scene_cfg.static_object_cfg = dict()
#     #     for name, cfg in scene_cfg_json["static_object_cfg"].items():
#     #        scene_cfg.static_object_cfg[name] = AssetBaseCfg(
#     #            spawn=usd_file_cfg_from_dict(cfg["spawn"])
#     #        )
    
#     # # RigidObject
#     # if "rigid_objects_cfg" in scene_cfg_json.keys():
#     #     scene_cfg.rigid_objects_cfg = dict()
#     #     for name, cfg in scene_cfg_json["rigid_objects_cfg"].items():
#     #        scene_cfg.rigid_objects_cfg[name] = RigidObjectCfg(
#     #            spawn=usd_file_cfg_from_dict(cfg["spawn"])
#     #        )

#     # # deformable objects
#     # if "deformable_objects_cfg" in scene_cfg_json.keys():
#     #     scene_cfg.deformable_objects_cfg = dict()
#     #     for name, cfg in scene_cfg_json["deformable_objects_cfg"].items():
#     #        scene_cfg.deformable_objects_cfg[name] = DeformableObjectCfg(
#     #            spawn=usd_file_cfg_from_dict(cfg["spawn"])
#     #        )
    
#     # scene_cfg.from_dict(scene_cfg_json)
#     return scene_cfg

# def get_empty_scene_cfg()->SceneCfg:
#     return SceneCfg(
#         num_envs=1,
#         env_spacing=4.0,
...
if __name__ == "__main__":
    ...
    data_dict = open("/home/admin01/桌面/Work/00-PsiLab/psi-lab-v2/outputs/tp/20250414_152601_scene_config.json", 'r')
    scene_dict = json.loads(data_dict.read())
    ...
    aa = scene_cfg(scene_dict)
    # bb= light_cfg(data_dic)
    # articulation_props = RigidObjectCfg(
    #     spawn=usd_file_cfg_from_dict(data_dic["spawn"])
    # )
