...
class RLEnv(gym.Env):
    ...
    # Change Scene Config From DirectRLEnvCfg To PsiRLEnvCfg
    # Author: Feng Yunduo
    # Time: 2025-04-16    
    # Code Bak: def __init__(self, cfg: DirectRLEnvCfg, render_mode: str | None = None, **kwargs):
    def __init__(self, cfg: RLEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        with Timer("[INFO]: Time taken for scene creation", "scene_creation"):
            # Change Scene Config From InteractiveScene To Scene
            # Author: Feng Yunduo
            # Time: 2025-04-16    
            # Code Bak: self.scene = InteractiveScene(self.cfg.scene)
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        for _ in range(self.cfg.decimation):
            ...
            # write_data_finish = time.time()
            

            # simulate
            self.sim.step(render=False)

            # sim_step_finish = time.time()
            ...
            self.scene.update(dt=self.physics_dt)
            # scene_update_finish = time.time()

            # print('write_data_finish:%s, sim_step_finish:%s, scene_update_finish:%s' % (
            # (write_data_finish - write_data_start)*1000,
            # (sim_step_finish - write_data_finish)*1000,
        ...
        if len(reset_env_ids) > 0:
            ...
            # write_data_to_sim_end = time.time()
            ...
            # print('reset_end:%s, write_data_to_sim_end:%s, forward_end:%s, render_end:%s 毫秒' % (
            #     (reset_end - reset_start)*1000,
            #     (write_data_to_sim_end - reset_end)*1000,
            #     (forward_end - write_data_to_sim_end)*1000,
            #     (render_end - forward_end)*1000
            #     )
            # )
        ...
        # get_obs_end = time.time()

        # print('pre_physics:%s, apply_action:%s, get_dones:%s, get_reward:%s,others:%s,get_obs:%s 毫秒' % (
        #     (pre_physics_end - start)*1000,
        #     (apply_action_end - pre_physics_end)*1000,
        #     (get_dones_end - apply_action_end)*1000,
        #     (get_reward_end - get_dones_end)*1000,
        #     (others_end - get_dones_end)*1000,
        #     (get_obs_end - others_end)*1000
        #     )
        # )
