...
class VuerTp():
    ...
    def __init__(self, cfg:VuerTpCfg):
        ...
        self.head_mat_default[:,3] = numpy.array(cfg.head_pos + tuple([1]))
        self.left_wrist_mat_default = numpy.eye(4)
        self.left_wrist_mat_default[:,3] = numpy.array(cfg.hand_right_offset + tuple([1]))
        self.right_wrist_mat_default = numpy.eye(4)
        self.right_wrist_mat_default[:,3] = numpy.array(cfg.hand_left_offset + tuple([1]))

        # 初始化 retarget 配置
        left_hand_cfg = cfg.left_hand_retarget_cfg
        right_hand_cfg = cfg.right_hand_retarget_cfg
    ...
    def step(self):
        """
        Update output from vr and set image of vr
        """
        pass
        # # 获取Vr输出
        # head_rmat, left_pose, right_pose, left_qpos, right_qpos = self.veur_step()
    ...
    def start(self,):
        ...
        self.vuer_thread = threading.Thread(target=self.run, daemon=True)
